...
class TestReadDedup:
    ...
    async def test_read_after_external_modification_returns_full(self, tool, tmp_path):
        ...
        # Modify the file externally — force mtime forward to avoid same-second collision
...
class TestReadDescriptionUpdate:
    ...
    def test_description_mentions_image_metadata(self):
        tool = ReadFileTool()
        desc = tool.description.lower()
        assert "image" in desc and "metadata" in desc
