...
def test_on_reaction_added_stores_reaction_id() -> None:
    """_on_reaction_added stores the returned reaction_id in _active_reactions."""
    channel = _make_feishu_channel()
    loop = asyncio.new_event_loop()
    try:
        task = loop.create_task(asyncio.sleep(0, result="reaction_abc"))
        loop.run_until_complete(task)
        channel._on_reaction_added("oc_chat1", "om_001", task)
    finally:
        loop.close()

    assert ("om_001", "reaction_abc") in channel._active_reactions["oc_chat1"]
...
def test_on_reaction_added_skips_none_result() -> None:
    ...
    assert "oc_chat1" not in channel._active_reactions
