...
@pytest.mark.asyncio
async def test_runspec_accepts_injection_callback():
    """AgentRunSpec should accept an optional injection_callback."""
    ...
    spec = AgentRunSpec(
        ...
        injection_callback=_drain,
    )

    assert spec.injection_callback is _drain
...
async def test_runner_injects_pending_messages_between_iterations():
    ...
    async def drain_callback() -> list[dict]:
        drain_count["n"] += 1
        if drain_count["n"] == 1:
            return [{"role": "user", "content": "please stop"}]
        return []
    ...
    result = await runner.run(AgentRunSpec(
        ...
        injection_callback=drain_callback,
    ))
