...
class TestAutoCompactIdleDetection:
    ...
    async def test_auto_compact_triggers_on_idle(self, tmp_path):
        ...
        assert any("new msg" in str(m.get("content", "")) for m in session_after.messages)
        await loop.close_mcp()
...
class TestProactiveAutoCompact:
    ...
    @pytest.mark.asyncio
    async def test_no_session_split_when_nothing_to_archive(self, tmp_path):
        """Session with ≤8 messages should NOT create a new SQLite session on archive."""
        loop = _make_loop(tmp_path, session_ttl_minutes=15)
        session = loop.sessions.get_or_create("cron:abc123")
        ...
        await loop.auto_compact._archive("cron:abc123")

        session_after = loop.sessions.get_or_create("cron:abc123")
        # db_id should NOT change — no split happened
        assert session_after.db_id == original_db_id
...
class TestSummaryPersistence:
    ...
    async def test_summary_recovered_after_restart(self, tmp_path):
        ...
        # _last_summary stays in metadata (non-destructive read).
    ...
    async def test_metadata_persists_for_restart(self, tmp_path):
        ...
        # _last_summary stays in metadata (non-destructive read).
        assert "_last_summary" in reloaded.metadata
        # Second call still returns the summary (persistent until overwritten).
