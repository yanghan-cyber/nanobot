...
class TestSecurityAttributeProtection:
    ...
    @pytest.mark.asyncio
    async def test_modify_bash_config_blocked(self):
        """bash_config is READ_ONLY — cannot be modified."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="bash_config", value=MagicMock())
        assert "read-only" in result
    ...
    @pytest.mark.asyncio
    async def test_inspect_bash_config_allowed(self):
        """bash_config is READ_ONLY — check should work."""
        tool = _make_tool()
        result = await tool.execute(action="check", key="bash_config")
        assert "Error" not in result
    ...
    @pytest.mark.asyncio
    async def test_modify_bash_config_dotpath_blocked(self):
        """bash_config.enable = False should be blocked because bash_config is READ_ONLY."""
        tool = _make_tool()
        result = await tool.execute(action="set", key="bash_config.enable", value=False)
        assert "read-only" in result
