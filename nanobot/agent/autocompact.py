...
class AutoCompact:
    ...
    async def _archive(self, key: str) -> None:
        try:
            ...
            if not archive_msgs:
                # Nothing to archive — skip split entirely.  Don't call
                # save() to avoid creating empty SQLite rows for sessions
                # with no meaningful content (e.g. short-lived cron jobs).
                return
    ...
    def prepare_session(self, session: Session, key: str) -> tuple[Session, str | None]:
        ...
        # Non-destructive read — summary stays in metadata so it survives restarts
        # and is re-injected each turn (same as upstream behavior).
