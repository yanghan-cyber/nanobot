...
_UNSET = object()
_webui_model_name: str | None | object = _UNSET


def _read_webui_model_name() -> str | None:
    """Return the configured default model for readonly webui display (cached)."""
    global _webui_model_name
    if _webui_model_name is not _UNSET:
        return _webui_model_name
    try:
        from nanobot.config.loader import load_config

        model = load_config().agents.defaults.model.strip()
        _webui_model_name = model or None
    except Exception as e:
        logger.debug("webui bootstrap could not load model name: {}", e)
        _webui_model_name = None
    return _webui_model_name
...
class WebSocketChannel(BaseChannel):
    ...
    def __init__(
        ...
        *,
        ...
        # Pre-compute normalized paths to avoid per-request string ops.
        self._normalized_token_issue_path: str | None = (
            _normalize_config_path(self.config.token_issue_path)
            if self.config.token_issue_path
            else None
        )
        self._normalized_ws_path: str = self._expected_path()
    ...
    @staticmethod
    def _purge_expired(pool: dict[str, float]) -> None:
        now = time.monotonic()
    ...
    def _purge_expired_issued_tokens(self) -> None:
        self._purge_expired(self._issued_tokens)
    ...
    async def _dispatch_http(self, connection: Any, request: WsRequest) -> Any:
        ...
        if self._normalized_token_issue_path:
            if got == self._normalized_token_issue_path:
        ...
        if got == "/api/sessions":
            return self._handle_sessions_list(request, query)
        ...
        if m:
            return self._handle_session_delete(request, m.group(1), query)
        ...
        if got == self._normalized_ws_path and _is_websocket_upgrade(request):
    ...
    def _check_api_token(self, request: WsRequest, query: dict[str, list[str]] | None = None) -> bool:
        """Validate a request against the API token pool (multi-use, TTL-bound)."""
        self._purge_expired_api_tokens()
        token = _bearer_token(request.headers) or (
            _query_first(query, "token") if query else None
        )
        if not token:
            return False
    ...
    def _purge_expired_api_tokens(self) -> None:
        self._purge_expired(self._api_tokens)
    ...
    def _handle_webui_bootstrap(self, connection: Any, request: Any) -> Response:
        ...
        return _http_json_response(
            {
                "token": token,
                "ws_path": self._normalized_ws_path,
                "expires_in": self.config.token_ttl_s,
                "model_name": _read_webui_model_name(),
            }
        )

    def _handle_sessions_list(self, request: WsRequest, query: dict[str, list[str]]) -> Response:
        if not self._check_api_token(request, query):
            return _http_error(401, "Unauthorized")
    ...
    def _resolve_webui_session(
        self, request: WsRequest, key: str, query: dict[str, list[str]],
    ) -> tuple[str, None] | tuple[None, Response]:
        """Authenticate and decode a session key for the webui surface.

        Returns ``(decoded_key, None)`` on success or ``(None, error_response)``
        on failure.
        """
        if not self._check_api_token(request, query):
            return None, _http_error(401, "Unauthorized")
        if self._session_manager is None:
            return None, _http_error(503, "session manager unavailable")
        ...
        return decoded_key, None

    def _handle_session_messages(self, request: WsRequest, key: str, query: dict[str, list[str]]) -> Response:
        decoded_key, err = self._resolve_webui_session(request, key, query)
        if err is not None:
            return err
    ...
    def _handle_session_delete(self, request: WsRequest, key: str, query: dict[str, list[str]]) -> Response:
        decoded_key, err = self._resolve_webui_session(request, key, query)
        if err is not None:
            return err
